	...
	public Map nativeUserChooseBusiness(String mid, String bid) {
		...
		try {
			...
			if(rp_list.size()>0){
				...
				List<RescueProcess> other_list = rescueProcessDao.findByHQL("from RescueProcess where mid = " + mid + "and bid <> " + bid);
				for(RescueProcess other_rp : other_list){
					other_rp.setStatus("failed");
					rescueProcessDao.updateRescueProcess(other_rp);
					RescueProcessHistory other_rph = new RescueProcessHistory();
					other_rph.setDate(df.format(date));
					other_rph.setRescueProcess(rp);
					other_rph.setStatus("failed");
					rescueProcessHistoryDao.addRescueProcessHistory(other_rph);
				}
